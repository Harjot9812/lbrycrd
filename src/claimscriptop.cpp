...
static std::string claimName(const std::vector<unsigned char>& name)
{
...
bool ProcessClaim(CClaimScriptOp& claimOp, CClaimTrieCache& trieCache, const CScript& scriptPubKey)
{
    ...
    case OP_CLAIM_NAME:
        ...
        return claimOp.claimName(trieCache, claimName(vvchParams[0]));
    case OP_SUPPORT_CLAIM:
        assert(vvchParams.size() == 2);
        return claimOp.supportClaim(trieCache, claimName(vvchParams[0]), uint160(vvchParams[1]));
    case OP_UPDATE_CLAIM:
        assert(vvchParams.size() == 3);
        return claimOp.updateClaim(trieCache, claimName(vvchParams[0]), uint160(vvchParams[1]));
    default:
        return false;
    }
}
